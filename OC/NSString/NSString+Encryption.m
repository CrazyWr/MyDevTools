...
+(NSString *)encryptSting:(NSString *)sText key:(NSString *)key andDesiv:(NSString *)iv
{
    ...
    const void *vinitVec = (const void *) [iv cStringUsingEncoding:NSASCIIStringEncoding];
    //CCCrypt函数 加密/解密
...
+ (NSString *)encrypt3DesSting:(NSString *)sText key:(NSString *)key andDesiv:(NSString *)iv{
    ...
    ccStatus = CCCrypt(kCCEncrypt,
                       ...
                       &movedBytes);
    //编码 byte
    NSData *basedata = [NSData dataWithBytes:(const void *)bufferPtr length:(NSUInteger)movedBytes];
    Byte *bytes = (Byte *)[basedata bytes];
    //下面是Byte 转换为16进制。
    NSString *hexStr=@"";
